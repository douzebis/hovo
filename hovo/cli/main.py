...
from hovo import cookies, dot_google, dot_user, glob, option
...
from hovo.cli.cli_check import check_cli
...
@click.option(
    ...
    default=False,
    help="""
        Retrieve doc information from cache, not from Google docs/drive
    """,
)
@click.option(
    '--show-xsrf-token',
...
def main_cli(
    ...
    option.load_from_cache = load_from_cache
    option.stop_on_warning = stop_on_warning
    option.traces = traces

    if option.load_from_cache and not option.dry_run:
        raise click.BadParameter(
            f"options --load-from-cache and --no-dry-run are incompatible") 
    ...
    if show_xsrf_token:
        ...
        Ansi.print(f"{get_b7r_xsrf_token()}")
        return
